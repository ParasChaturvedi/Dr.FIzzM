...
export default function Steps({ currentStep = 1 }) {
  ...
  useEffect(() => {
    const measure = () => {
      ...
      const top = firstRect.top - wrapRect.top + firstRect.height / 2 - DOT_DIAM / 2;
  ...
  // how far the blue overlay should extend
  const maxSegments = steps.length - 1;
  const progressPct =
    maxSegments > 0 ? Math.min(Math.max((currentStep - 1) / maxSegments, 0), 1) : 0;
  ...
  return (
    <div className="w-full max-w-4xl mx-auto mt-6 mb-8">
      ...
      <div ref={wrapRef} className="relative flex items-center justify-center gap-x-10">
        {/* gray base dots (full length) */}
        <div
          aria-hidden
          style={{
            position: "absolute",
            ...
            top: `${linePos.top}px`,
            height: `${DOT_DIAM}px`,
            backgroundImage: `radial-gradient(circle, #cbd5e1 ${DOT_DIAM / 2}px, transparent ${DOT_DIAM / 2}px)`,
        ...
        />
        {/* blue progress overlay (animated width) */}
        <div
          aria-hidden
          style={{
            position: "absolute",
            ...
            top: `${linePos.top}px`,
            height: `${DOT_DIAM}px`,
            backgroundImage: `radial-gradient(circle, #2563eb ${DOT_DIAM / 2}px, transparent ${DOT_DIAM / 2}px)`,
            backgroundSize: BG_SIZE,
            backgroundRepeat: "repeat-x",
            backgroundPosition: "0 center",
            pointerEvents: "none",
            transition: "width 600ms cubic-bezier(.2,.8,.2,1), left 250ms ease, top 250ms ease",
          }}
        />
        ...
        {steps.map((step, i) => {
          ...
          return (
            <div key={step.id} className="flex flex-col items-center" style={{ minWidth: 104 }}>
              {/* circle */}
              <div
                ref={(el) => (circleRefs.current[i] = el)}
                className={`flex items-center justify-center rounded-full w-9 h-9 z-10 transition-all duration-300
                  ${isActive ? "bg-blue-500 text-white scale-105 shadow-sm pulse" : "bg-gray-200 text-gray-600 "}`}
              >
                {isCompleted ? (
                  // ✅ “drawn” checkmark animation via stroke-dashoffset
                  <svg
                  ...
                  <span className="font-semibold text-sm transition-colors duration-300">{step.id}</span>
                )}
              </div>
              ...
              <span
                className={`mt-2 text-sm text-center leading-tight transition-colors duration-300
                  ${isActive ? "font-medium text-gray-900 dark:text-white" : "text-gray-600 dark:text-gray-300"}`}
              >
                {step.label}
              </span>
            </div>
